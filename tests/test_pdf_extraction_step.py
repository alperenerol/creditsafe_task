...
class TestPDFExtraction(unittest.TestCase):
    ...
    def test_existing_raw_text_file(self, mock_exists, mock_get_raw_text_path):
        ...
        mock_get_raw_text_path.return_value = os.path.join(
            self.test_dir_path, self.raw_text_file
        )
        mock_exists.return_value = True

        # Mock opening and reading from the existing raw text file
        with patch(
            "builtins.open", mock_open(read_data="Existing raw text")
        ) as mock_file:
            ...
            mock_file.assert_called_once_with(
                mock_get_raw_text_path.return_value, "r", encoding="utf-8"
            )
    ...
    def test_extract_text_success(
        self, mock_get_raw_text_path, mock_exists, mock_save_text, mock_extract_text_ocr
    ):
        ...
        self.assertEqual(pdf_file, self.pdf_file)
        mock_extract_text_ocr.assert_called_once_with(
            os.path.join(self.test_dir_path, self.pdf_file)
        )
        mock_save_text.assert_called_once_with(
            mock_get_raw_text_path.return_value, "Extracted text content"
        )
    ...
    def test_extract_text_failure(
        self, mock_get_raw_text_path, mock_exists, mock_extract_text_ocr
    ):
