...
class TestTextPreprocessing(unittest.TestCase):
    ...
    def test_preprocessed_file_exists(
        ...
        with patch(
            "builtins.open", mock_open(read_data=self.preprocessed_text_content)
        ) as mock_file:
            preprocessed_text = raw_text_preprocess(
                self.raw_text_content, self.pdf_file, text_dir=self.test_dir_path
            )
            self.assertEqual(preprocessed_text, self.preprocessed_text_content)
            mock_file.assert_called_once_with(
                preprocessed_text_path, "r", encoding="utf-8"
            )
    ...
    @patch("os.path.exists")
    @patch("src.preprocess.preprocessing_step.get_preprocessed_text_path")
    def test_successful_preprocessing(
        self,
        mock_get_preprocessed_text_path,
        mock_exists,
        mock_save_text,
        mock_process_text,
    ):
        ...
        mock_save_text.assert_called_once_with(
            preprocessed_text_path, self.preprocessed_text_content
        )
    ...
    def test_preprocessing_failure(
        self, mock_get_preprocessed_text_path, mock_exists, mock_process_text
    ):
